...
public class Disciplina {
    ...
    String[] listaNomes = {"joão","maria","carlos","ana","pedro","alberto","kaue","cleber","julia","sara"};
    
    public void populaArray(){
        for (int i = 0; i < 10; i++) {
           adicionaArray(listaNomes[i], gerador1a10Ids(), i, gerador0a10Notas(), gerador0a10Notas());          
        }
 
    }
    
    public void adicionaArray(String nome, int id, int posição, double nota1, double nota2) {
    ...
    public void antecessorSucessor(int posição) {
        if (posição == 9){
            System.out.println("o aluno antecessor é: " + alunos[(posição - 1)].nome);
            System.out.println("Não há aluno sucessor");
        }
        if (posição == 0){ 
            System.out.println("Não há aluno antecessor");
            System.out.println("O aluno sucessor é: " + alunos[(posição + 1)].nome);
        }
